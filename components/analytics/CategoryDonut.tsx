...
function DonutTip({
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-3 px-3 py-2 shadow-pop">
      <p className="flex items-center gap-1.5 text-xs font-semibold text-ink">
        <span>{s.emoji}</span>
        {s.label}
      </p>
      <p className="tabular mt-0.5 text-xs text-ink-dim">{fmtMoney(s.value)}</p>
    </div>
  );
}
...
export default function CategoryDonut({
  ...
  const empty = total === 0;
  const pieData = empty
    ? [{ id: "empty", label: "No spending", emoji: "", color: "var(--surface-3)", value: 1 }]
    : nonZero;
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <div key={cat.id} className="flex items-center gap-2.5">
              ...
              <span className="tabular w-10 shrink-0 text-right text-[11px] text-ink-faint">
